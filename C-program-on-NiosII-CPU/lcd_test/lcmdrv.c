...
{ uint32  i;
  ...
  LCM_WrCommand(0xaf);		//LCD On
	LCM_WrCommand(0x2f);		//设置上电控制模式
    LCM_WrCommand(0x27);//______________
	LCM_WrCommand(0x81);		//电量设置模式（显示亮度）
	LCM_WrCommand(0x3f);		//指令数据0x0000~0x003f

	//LCM_WrCommand(0x25);		//V5内部电压调节电阻设置
	LCM_WrCommand(0xa3);		//LCD偏压设置，V3时选

	LCM_WrCommand(0xc0);		//Com 扫描方式设置
	LCM_WrCommand(0xa1);		//Segment方向选择
	LCM_WrCommand(0xa4);		//全屏点亮/变暗指令
	LCM_WrCommand(0xa6);		//正向反向显示控制指令
	LCM_WrCommand(0xac);		//关闭静态指示器
	LCM_WrCommand(0x00);		//指令数据
