...
****************************************************************************************/
//modified by xiaofei  for niosii
...
#if  FONT5x7_EN==1
uint8 const  DEC_HEX_TAB2[8] = {0x80, 0x40, 0x20, 0x10, 0x08, 0x04, 0x02, 0x01};
...
{  uint8   font_dat;
   ...
   {  /* 读取点阵数据 */
      ...
      /* if( (font_dat&DCB2HEX_TAB[j])==0 ) GUI_CopyColor(&bakc,back_color);
             ...
             if( (font_dat&DEC_HEX_TAB2[j])==0 ) bakc=BACK_COLOR_MY;
            else  bakc=DISP_COLOR_MY;
         GUI_Point(x, y, bakc);       
         x++;
      }
...
****************************************************************************/
/*
void  GUI_PutString1(uint32 x, uint32 y, char *str,uint8 ptos1,uint8 ptos2)
{
   uint8 i=0;
   while(1)
   {  if( (*str)=='\0' ) 
     {
	    ...
	    if(i==ptos1||i==(ptos2+1)) GUI_ExchangeColor();		   
